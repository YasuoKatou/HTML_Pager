...
|1-3|[カテゴリ更新](#カテゴリ更新)|update_category|
|1-4|[カテゴリ削除](#カテゴリ削除)|delete_category|
|1-5|[カテゴリ移動](#カテゴリ移動)|move_category|
...
|4-3|[タグ更新](#タグの更新)|update_tag|
|4-4|[タグ削除](#タグの削除)|delete_tag|
|4-5|[TODOタグの登録](#todoタグの登録)|set_todo_tag|
...
- リクエスト（read_category）  
  ...
  |No|項目名称|物理名|型|説明|
  |:-:|:--|:--|:--|:--|
  |1|タイプ|type|数字|1:カテゴリ一覧、2:カテゴリ一覧＋集計|
...
- レスポンス（リクエスト.type = 1）
  ...
  |1|カテゴリ情報|category_list|配列|(1)参照|
    ...
    |1|カテゴリID|id|数字||
    |2|カテゴリ名|name|文字列||
    |3|未着手数|num1|数字|0固定（*1）|
    |4|着手数|num2|数字|0固定（*1）|
    |5|完了数|num3|数字|0固定（*1）|

    *1：「リクエスト.type = 2」と同じフォーマットにする
...
- レスポンス  
  カテゴリ取得(type=2)．レスポンスと同じ

[↑ 外部インターフェース一覧](#外部インターフェース一覧)

### カテゴリ更新

- リクエスト（update_category）
...
- リクエスト（move_category）
  ...
  |2|移動元カテゴリID|category_id_fm|数字||
  |3|移動先カテゴリID|category_id_to|数字||
